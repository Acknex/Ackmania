...
var get_line_segment_point_dist(VECTOR* vec1, VECTOR* vec2, VECTOR* point, VECTOR* vresult, var* t)
{
   ...
   else
   {
      ...
      if (dist1 < dist2)
         ...
         res = dist1;
         if (vresult)
            vec_set(vresult, vec1);
      }
      else
      {
         res = dist2;
         if (vresult)
            vec_set(vresult, vec2);
      }
   }
...
void path_get_offset_position(VECTOR* vdata, var offset, VECTOR* vresult)
{
   ...
   t = offset / length;
   if (offset >= 0)
      k = vdata.y + floor(t + vdata.z);
   else
      k = vdata.x + floor(t + vdata.z);
   if (k <= 0)
      k = max_nodes;
   if (k > max_nodes)
      k = 1;
...
void path_get_normal_position(VECTOR* vpos, var path_offset, var offset, VECTOR* vresult)
{
   ...
   if (path_offset)
      path_get_offset_position(temp2, path_offset, temp);
   ...
   if (ang1 > 360 && ang1 > ang2)
      ang1 -= 360; // dirty fix
   if ((ang1 > ang2 && offset < 0) || (ang1 < ang2 && offset > 0))
      vec_inverse(vnormal);
...
var ent_path_get_progress(ENTITY* kart, var* length)
{
   ...
   *length = 0;
   ...
   for (i = 1; i < max_nodes; i++)
      ent->skill[i + 1] = ent->skill[i + 1] + ent->skill[i];
...
void loadPlayerCpuControlParams(ENTITY* ent)
{
   ...
   if (abs(angle_diff) > 0)
         {
      if (angle_diff > 0)
         ent->kart_input |= INPUT_LEFT;
      if (angle_diff < 0)
         ent->kart_input |= INPUT_RIGHT;
   }
}
...
void updatePlayer(ENTITY* ent)
{
   ...
   if (ent->falling)
   {
      ...
      if (up > ent->falling)
         ent->falling += (up - ent->falling) * 0.2 * time_step;
      else
         ent->falling += (up - ent->falling) * 0.15 * time_step;
      ...
      ang_rotate(ent->parent->pan,
            vector(0, (-(ent->falling_dir == 0) + (ent->falling_dir == 1)) * 15 * time_step,
                  (-(ent->falling_dir == 2) + (ent->falling_dir == 3)) * 15 * time_step));
   ...
   ent->speed = minv(ent->speed,
         (ent->kart_maxspeed
               - (1 - ent->kart_drift_buffer) * ent->kart_maxspeed * 0.5 * abs(ent->turn_speed2) / g_raceplayerTurnSpeed * !ent->drifting)
               * ent->underground);
   ...
   if (!up && down) {
      ent->speed = maxv(ent->speed - (0.5 + 0.5 * (ent->speed > 0)) * g_raceplayerAccelSpeed * 2 * time_step,
            -g_raceplayerBreakForce * !ent.drifting);
   }
   ...
   c_trace(vector(ent->x, ent->y, ent->z + 64), vector(ent->x, ent->y, -128),
         IGNORE_PASSABLE | IGNORE_PUSH | SCAN_TEXTURE | USE_POLYGON | IGNORE_SPRITES);
   ...
   if (!ent->drifting) {
      ...
      ent->drift_pan += clamp((ent->drift_dir * (3 + 15 * abs(left - right) * (sign(left - right) == ent->drift_dir)) - ent->drift_pan) * 0.4, -3.5,
            3.5)
            * time_step;
   }
   ...
   if (ent->speed < 15 || ent->underground < 0.9)
      ent->drifting = 0;
   if (ent->ground_contact && ent->speed >= ent->kart_maxspeed * 0.2) {
      ...
      if (ent->particle_emit > 0.5)
         ...
         {
            c_ignore(group_kart, 0);
            c_trace(vector(temp.x, temp.y, temp.z + 64), vector(temp.x, temp.y, -128),
                  IGNORE_PASSABLE | IGNORE_PUSH | SCAN_TEXTURE | USE_POLYGON | IGNORE_SPRITES);
            if (hit.green > 100)
               effect(p_kart_grass, 1, temp, hit.blue); //temp2);
         }
         vec_set(temp, vector(-22, -22, -ent->kart_height * 0.5));
         vec_rotate(temp, ent->parent->pan);
         vec_add(temp, ent->parent->x);
   ...
   if (item) {
      switch (ent->item_id) {
         ...
         case ITEM_ROCKET:
            shoot_rocket(ent);
            break;
         case ITEM_AIM_ROCKET:
            shoot_aiming_rocket(ent);
            break;
         case ITEM_TURBO:
            use_turbo(ent);
            break;
         case ITEM_BADASS_ROCKET:
            shoot_badass_aiming_rocket(ent);
            break;
         case ITEM_MUSHROOM:
            start_mushroom(ent);
            break;
   ...
   if (ent->kart_progress_update > 2)
      ...
      if (ent->kart_lap > 0)
         ...
         if (progress + up > ent->kart_progress + up)
            ent->kart_progress = progress + up;
         if (progress + up < ent->kart_progress + up - 512)
            ent->kart_checkpoint = -1;
      }
   }
      ...
      if (ent->kart_bot_separate > 4) {
         ...
         for (i = 2; i <= 4; i++) {
            ...
            if (i != ent->sk_kart_id) {
               ...
               if (vec_length(temp) < 1) {
                  vec_randomize(temp, 128);
               } else {
